...
<article class="container products-archive">
    ...
    <div class="filtro">
      ...
      <form class="filtro-preco">
        <div>
          ...
          <input type="text" name="min_price" id="min_price" required value="<?php $_GET['min_price'] ?? ''; ?>" />
        </div>
        <div>
          <label for="max_price">Até R$</label>
          <input type="text" name="max_price" id="max_price" required value="<?php $_GET['max_price'] ?? ''; ?>" />
        </div>
  ...
  <main>
    ...
    <?php hendal_product_list($data['products']); ?>
